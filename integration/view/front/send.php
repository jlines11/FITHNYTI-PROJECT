...
if (isset($_POST["reset"])) {
          ...
          <table width="600px">
           <tr>
             <td>
                ...
                A bientôt sur <a href="http://localhost:3000/integration/view/front/index.php">FITHNYTI</a> !
    ...
    ';
        ;
        $mail->send();
        header("Location: http://localhost:3000/integration/view/front/send.php?section=code");
...
if (isset($_POST['verif_submit'], $_POST['verif_code'])) {
    if (!empty($_POST['verif_code'])) {
        ...
        if ($verif_req == 1) {
            ...
            header('Location: http://localhost:3000/integration/view/front/send.php?section=changemdp');
            exit;
        } else {
            $error = "Code invalid";
            header("Location: http://localhost:3000/integration/view/front/send.php?section=code&error=" . urlencode($error));
            exit;
        }
    }
}
            ...
            if (!empty($mdp) && !empty($mdpc)) {
                if ($mdp == $mdpc) {
                    ...
                    if ($ins_mdp->execute(array($hashedPassword, $_SESSION['email']))) {
                        ...
                        header('Location: http://localhost:3000/integration/view/front/login.php');
                        exit;
                    } else {
